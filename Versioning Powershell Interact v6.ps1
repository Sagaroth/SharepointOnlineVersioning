...
function Supprimer-Historique-Versions-Bibliotheque {
    ...
    try {
        ...
        $batchCounter++
        Write-Host "Traitement du lot $batchCounter/$totalBatches en cours..." -ForegroundColor Cyan
        $fichiersTraites = 0
    ...
    foreach ($file in $batches) {
        ...
        try {
            ...
            Write-Host "Une erreur s'est produite lors de l'accès aux propriétés de $fileDir/$fileName. Détails: $_ Ignoré." -ForegroundColor Red
            continue
        }
        ...
        do {
            try {
                ...
                if ($_.Exception.Message -like "*The collection has not been initialized*") {
                    ...
                    $delta = $versions.Count - $versionsAConserver  # Recalculer le delta pour s'assurer de traiter le bon nombre de versions
                } else {
